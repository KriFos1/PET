...
class Ensemble:
    ...
    def _ext_ml_info(self):
        ...
        if 'multilevel' in self.keys_en:
            ...
            for i, opt in enumerate(list(zip(*self.keys_en['multilevel']))[0]):
                ...
                if opt == 'scaling':
                    self.multilevel['scaling'] = [el
                                             for el in self.keys_en['multilevel'][i][1].split(' ')]
                if opt == 'scaling_map':
                    self.multilevel['scaling_map'] = [el
                                             for el in self.keys_en['multilevel'][i][1].split(' ')]
    ...
    def calc_ml_prediction(self, input_state=None):
        ...
        no_tot_run = int(self.sim.input_dict['parallel'])
        ml_pred_data = [[] for _ in range(max(self.multilevel['levels'])+1)] # Ensure that the list is long enough
